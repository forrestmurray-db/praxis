...
# TODO
data_version = 0
train_delta = spark.read.format("delta").load(train_delta_path)
test_delta = spark.read.format("delta").load(test_delta_path)
...
v0 = mlflow.search_runs(filter_string=f"params.data_version={data_version}")
# dv_metrics.append(r["metrics.rmse"], r["metrics.r2"] for r in v0)
...
v1 = mlflow.search_runs(filter_string=f"params.data_version={data_version}")

# COMMAND ----------

dv_metrics = v0.append(v1, ignore_index=True)[["params.data_version", "metrics.rmse", "metrics.r2"]]

# COMMAND ----------

dv_metrics.sort_values("metrics.rmse")
