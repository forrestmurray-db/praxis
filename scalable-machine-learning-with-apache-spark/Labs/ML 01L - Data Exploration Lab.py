...
train_df = train_df.withColumn("log_price", F.log(train_df.price))
...
# TODO
review_ct = train_df.groupBy('neighbourhood_cleansed').agg(F.sum('number_of_reviews').alias('n_reviews')).sort('n_reviews', ascending=False)
display(review_ct)
...
train_avg    = train_df.select(F.avg("price")).first()[0]
train_median = train_df.select(F.median("price")).first()[0]
print(f"training set average: {train_avg}, training set median: {train_median}")

# COMMAND ----------

pred_df = test_df.withColumn("avgPrediction", F.lit(train_avg)).withColumn("medianPrediction", F.lit(train_median))
