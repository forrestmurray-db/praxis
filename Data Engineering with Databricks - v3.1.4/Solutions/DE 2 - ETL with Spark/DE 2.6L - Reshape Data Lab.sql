...
-- MAGIC %python
-- MAGIC ens = spark.sql("SELECT DISTINCT event_name FROM events").collect()
-- MAGIC ", ".join([ f"'{r['event_name']}'" for r in ens])
-- MAGIC # ",".join(ens)
...
SELECT * FROM (
  SELECT user_id user, event_name FROM events
    ...
    count(*) FOR event_name in ('mattresses', 'down', 'press', 'shipping_info', 'main', 'warranty', 'finalize', 'login', 'faq', 'careers', 'pillows', 'original', 'cart', 'delivery', 'cc_info', 'guest', 'add_item', 'email_coupon', 'premium', 'register', 'checkout', 'reviews', 'foam')
  )
...
SELECT * FROM transactions tx JOIN events_pivot ep ON ep.user = tx.user_id
