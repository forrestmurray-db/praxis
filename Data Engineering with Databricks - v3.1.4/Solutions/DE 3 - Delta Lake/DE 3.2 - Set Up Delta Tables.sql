...
-- MAGIC %python 
-- MAGIC files = dbutils.fs.ls(f"{DA.paths.working_dir}/database.db/purchases_clone")
-- MAGIC display(files)
...
-- MAGIC %python
-- MAGIC files = dbutils.fs.ls(f"{DA.paths.working_dir}/database.db/purchases_shallow_clone")
-- MAGIC display(files)
...
ALTER TABLE purchases_shallow_clone 
...
ALTER TABLE purchases_clone 
ADD COLUMN (another_col DOUBLE)
