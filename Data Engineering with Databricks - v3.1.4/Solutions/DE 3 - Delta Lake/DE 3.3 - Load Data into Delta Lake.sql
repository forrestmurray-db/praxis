...
-- MAGIC %python 
-- MAGIC datasets = dbutils.fs.ls(DA.paths.datasets)
-- MAGIC display(datasets)
...
-- MAGIC eventsDelta = DeltaTable.forPath(spark, 'dbfs:/mnt/dbacademy-users/forrest.murray@databricks.com/data-engineer-learning-path/database.db/events')
-- MAGIC location = eventsDelta.detail().first()['location']
-- MAGIC
-- MAGIC # dbutils.fs.ls(location + '/_delta_log')
-- MAGIC with open(location.replace('dbfs:/', '/dbfs/') + '/_delta_log/00000000000000000001.json', 'r') as log: 
-- MAGIC   print(log.read())
